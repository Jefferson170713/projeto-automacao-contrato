...
class MainWindow(QMainWindow):
    ...
    def criar_recisao_com_aviso(self):
        ...
        for num, nome in enumerate(self.df['NOME']):
            dict_modification = {
                ...
                "[NOME_CONTRATADA]" : self.df['NOME'].iloc[num],
            ...
            name_arquivo = f'RECISÃO CONTRATUAL - HAP - NOTIFICAÇÃO COM AVISO - {nome} - {name_mes}.docx'
            caminho_completo = os.path.join(self.path_folder, name_arquivo)
            doc.save(caminho_completo)
            bar_progresso += 1
            self.progress_bar.setValue(int((bar_progresso / bar_progresso_max) * 100))
    ...
    def criar_recisao_sem_aviso(self):
        ...
        for num, nome in enumerate(self.df['NOME']):
            ...
            name_arquivo = f'RECISÃO CONTRATUAL - HAP - NOTIFICAÇÃO SEM AVISO - {nome} - {name_mes}.docx'
            caminho_completo = os.path.join(self.path_folder, name_arquivo)
            doc.save(caminho_completo)
            bar_progresso += 1
            self.progress_bar.setValue(int((bar_progresso / bar_progresso_max) * 100))
